/*
We are given a directed graph. We need compute whether the graph has negative cycle or not. A negative cycle is one in which the overall sum of the
...
/*
This C++ program checks if there is a negative weight cycle in a given graph using the Floyd Warshall Algorithm. The program defines the number of vertices, infinite value (for unconnected vertices), and includes functions to perform the algorithm.

The negCycleFloydWarshall function implements the Floyd Warshall Algorithm to find the shortest distances between all pairs of vertices. If any vertex has a negative distance from itself at the end of the algorithm, it indicates a negative weight cycle in the graph.

The main function creates an example graph using an adjacency matrix and checks whether the graph contains a negative weight cycle or not. The program then outputs the result accordingly.
