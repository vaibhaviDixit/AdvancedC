...
/*
This C++ program demonstrates how to clone a directed acyclic graph. The program defines a Node class representing nodes in the graph, and it includes functions to print the graph depth-wise and to clone the graph while maintaining the same structure.

The cloneGraph function uses a depth-first traversal approach to recursively clone the original graph. It creates a new instance of Node for each unvisited node in the original graph, maintains the adjacency relationships, and continues the cloning process.

The main function creates an example directed acyclic graph, clones it, and prints the original and cloned graphs for comparison.
