/*
Given a directed graph represented by an adjacency matrix, find the transitive closure of the graph using Warshall's algorithm.
...
/*
This C program implements the Floyd-Warshall algorithm to compute the transitive closure of a directed graph.
The transitive closure of a graph indicates whether there is a path from each vertex to every other vertex in the graph.
The program takes an adjacency matrix of a graph as input and outputs the transitive closure matrix.
