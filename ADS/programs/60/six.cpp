/*
 Given a weighted directed graph represented by an adjacency matrix,
 determine if the graph contains any negative cycles using Warshall's algorithm.
...
bool negCyclefloydWarshall(int graph[][V])
{
	...
	graph matrix. Or we can say the initial values of shortest distances are based on shortest
...
/*
 This program demonstrates the use of the Floyd-Warshall algorithm to detect negative weight cycles in a directed graph.
The input is given in the form of an adjacency matrix with weighted edges. The program initializes the dist matrix to store the
shortest distances between vertices and updates it based on the Floyd-Warshall algorithm. It then checks if any diagonal element of
the dist matrix is negative, indicating the presence of a negative weight cycle. The program prints whether the graph contains a
